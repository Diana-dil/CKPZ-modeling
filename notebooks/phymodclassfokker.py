# -*- coding: utf-8 -*-
"""PhyModClassFokker.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1C_a3qdjFTBZL2yJ0jTlXHbZimZebk1lt
"""
...
class CKPZTransport:
    def __init__(self, L=1.0, nx=64, ny=None,
        ...
        if not np.isclose(self.dx, self.dy):
            raise ValueError("Требуется квадратная сетка (dx = dy)")
        ...
        # dt по умолчанию (будет переопределено в run_single)
    ...
    def _compute_wave_numbers(self):
        kx = 2.0 * np.pi * np.fft.fftfreq(self.nx, d=self.dx)   # (nx,)
        ky = 2.0 * np.pi * np.fft.fftfreq(self.ny, d=self.dy)   # (ny,)
        self.KX = kx[np.newaxis, :]      # (1, nx)
        self.KY = ky[:, np.newaxis]      # (ny, 1)
        self.K2 = self.KX**2 + self.KY**2  # (ny, nx)
        self.K4 = self.K2**2
        self.IKX = 1j * self.KX
        self.IKY = 1j * self.KY

    def _update_operators(self):
        """Коэффициенты ETD1 с правильным учётом шума."""
        # Избегаем деления на ноль для нулевой моды
        nuK4 = self.nu * self.K4
        # Маска для ненулевых K4
        mask = nuK4 > 1e-15
        self.Lin_fac = np.exp(-nuK4 * self.dt)          # E = exp(-L*dt)
        self.Nl_fac = np.where(mask,
                               (1.0 - self.Lin_fac) / nuK4,
                               self.dt)                  # (1-E)/L
        # Фактор для шума: sqrt( D*K2 * (1-exp(-2*L*dt)) / (2*L) )
        self.noise_fac = np.where(mask,
                                  np.sqrt(self.D * self.K2 * (1.0 - np.exp(-2.0 * nuK4 * self.dt)) / (2.0 * nuK4)),
                                  0.0)
        self.noise_fac[0,0] = 0.0   # нулевая мода

    def _init_state(self):
        self.h = 0.01 * self.rng.normal(size=(self.ny, self.nx))
        self.h_hat = np.fft.fft2(self.h)

        self.x_p = self.rng.uniform(0, self.L, self.N)
        self.y_p = self.rng.uniform(0, self.L, self.N)
        self.x0 = self.x_p.copy()
        self.y0 = self.y_p.copy()
        self.x_unwrap = self.x_p.copy()
        self.y_unwrap = self.y_p.copy()

    def _grad_interp(self, field):
        fpad = np.pad(field, 1, mode='wrap')
        dfdx = (fpad[1:-1, 2:] - fpad[1:-1, :-2]) / (2 * self.dx)
        dfdy = (fpad[2:, 1:-1] - fpad[:-2, 1:-1]) / (2 * self.dy)

        xg = np.linspace(0, self.L, self.nx, endpoint=False)
        yg = np.linspace(0, self.L, self.ny, endpoint=False)
        interp_dx = RegularGridInterpolator((yg, xg), dfdx,
                                            bounds_error=False, fill_value=None)
        interp_dy = RegularGridInterpolator((yg, xg), dfdy,
                                            bounds_error=False, fill_value=None)
        pts = np.column_stack((self.y_p % self.L, self.x_p % self.L))
        return interp_dx(pts), interp_dy(pts)

    def _step(self):
        """Корректный псевдоспектральный шаг ETD1."""
        h_hat = self.h_hat

        # Нелинейность: F[∇²(∇h)²] = -K2 * F[(∇h)²]
        dhdx = np.fft.ifft2(self.IKX * h_hat).real
        dhdy = np.fft.ifft2(self.IKY * h_hat).real
        grad_sq = dhdx**2 + dhdy**2
        N_hat = -self.K2 * np.fft.fft2(grad_sq)   # = F[-∇²(∇h)²]

        # Шум в фурье-пространстве
        xi = self.rng.normal(size=(self.ny, self.nx))
        xi_hat = np.fft.fft2(xi)

        # Схема: h_hat = Lin_fac * h_hat + Nl_fac * ( (λ/2)* N_hat ) + noise_fac * xi_hat
        h_hat = (self.Lin_fac * h_hat
                 + self.Nl_fac * (0.5 * self.lam * N_hat)
                 + self.noise_fac * xi_hat)
        self.h_hat = h_hat
        self.h = np.fft.ifft2(h_hat).real

        # Частицы
        grad_x, grad_y = self._grad_interp(self.h)
        dW = np.sqrt(2 * self.kappa * self.dt) * self.rng.normal(size=(2, self.N))
        dx = -self.gamma * grad_x * self.dt + dW[0]
        dy = -self.gamma * grad_y * self.dt + dW[1]

        self.x_p = (self.x_p + dx) % self.L
        self.y_p = (self.y_p + dy) % self.L
        self.x_unwrap += dx
        self.y_unwrap += dy
    ...
    def run_single(self, T_steps, record_every=50, max_frames=80,
        ...
        # иначе остаётся self.dt по умолчанию (из __init__)
        ...
        loop = tqdm(range(T_steps), desc="Интегрирование") if verbose else range(T_steps)
        for n in loop:
            self._step()

            if n % record_every == 0:
                W_vals.append(np.std(self.h))
                msd_vals.append(np.mean((self.x_unwrap - self.x0)**2 +
                                        (self.y_unwrap - self.y0)**2))
                t_vals.append(n * self.dt)

            if n == next_frame and len(frames_h) < max_frames:
                frames_h.append(self.h.copy())
                frames_p.append((self.x_p.copy(), self.y_p.copy()))
    ...
    def run_ensemble(self, n_runs, T_steps, record_every=50,
                     dt=None, max_time=None, verbose=True):
        all_W, all_msd = [], []
        for run_idx in tqdm(range(n_runs), desc="Ансамбль"):
    ...
    def run_varying_L_ensemble(self, L_list, n_runs, T_steps,
        ...
        W_means, W_stds = [], []
        times_common = None
        for L_val in tqdm(L_list, desc="Вариация L"):
            # Создаём экземпляр с таким же числом узлов, но другим L
            model = CKPZTransport(L=L_val, nx=self.nx, nu=self.nu, lam=self.lam,
    ...
    def animate(self, results, cmap='plasma', interval=80):
        ...
        im = ax1.imshow(frames_h[0], origin='lower', extent=extent,
                        cmap=cmap, interpolation='bilinear')
        ax1.set_title('Поверхность h(x,y)')
        ax1.set_xlabel('x'); ax1.set_ylabel('y')
        plt.colorbar(im, ax=ax1, fraction=0.046)

        x0, y0 = frames_p[0]
        ax2.hist2d(x0, y0, bins=50, range=[[0, self.L], [0, self.L]],
                   cmap='hot', density=True)
        ax2.set_title('Плотность частиц')
        ax2.set_xlabel('x'); ax2.set_ylabel('y')

        def update(frame_idx):
            im.set_array(frames_h[frame_idx])
            ax2.clear()
            ax2.set_title('Плотность частиц')
            ax2.set_xlabel('x'); ax2.set_ylabel('y')
            xp, yp = frames_p[frame_idx]
            ax2.hist2d(xp, yp, bins=50, range=[[0, self.L], [0, self.L]],
                       cmap='hot', density=True)
            return [im]
...
if __name__ == '__main__':
    model = CKPZTransport(L=1.0, nx=64, nu=1.0, lam=1.0, D=0.1,
                         kappa=0.01, gamma=0.1, N_particles=2000)

    # Одиночный прогон с заданным физическим временем
